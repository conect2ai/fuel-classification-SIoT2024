...
float compute_co2(float maf, int fuel_type)
{
  if (fuel_type == 0){ // gasoline
    int fuel_density = 737;         // (g/L)
    int emission_factor_CO2 = 2310; // (g/L)
    float afr = 14.7;

    float fuel_volume = maf / (afr * fuel_density); // (L)

    return fuel_volume * emission_factor_CO2; // (g)
  } else { // ethanol
    int fuel_density = 789;         // (g/L)
    int emission_factor_CO2 = 1510; // (g/L)
    float afr = 9;
...
void processOBD(CBuffer* buffer)
{
  ...
  // if (timestamp > 1711040023) {
  //   // add timestamp to buffer
  //   buffer->add(time_session_flag, ELEMENT_UINT32, &timestamp, sizeof(timestamp));
  // } else {
  //   return;
  // }
...
void setup()
{
  ...
  Serial.println("___TENSORFLOW MODEL___");
